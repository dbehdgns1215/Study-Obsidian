...
> 이 인덱스에는 employee의 타입(테이블의 개념)과 department의 타입이 존재하고 있고, 이들은 서로 독립적일 것이라고 당연하게 생각이 되지만 실제 Lucene에서 처리할 때는 타입 구분이 무시되고 같은 필드(컬럼 또는 열의 개념)로 처리되는 상황.
> 
> 즉,
...
>    
> 결론적으로, 매핑 타입은 멀티 테이블처럼 보이기 위한 ES의 '가짜 껍데기'였을 뿐이며, 루씬의 물리적 구조(인덱스-필드)와 1대1로 매칭되지 않아 성능 저하와 혼란을 야기했기 때문에 8.0에서 완전히 퇴출당함. 지금은 1 인덱스 = 1 테이블 구조가 강제됨.
