...
하지만 여러 토큰 필터를 적용할 때는 순서가 중요함.
위 예시의 경우, `lowercase`가 `stop`보다 먼저 적용됐는데, 만약 그 반대였다면 `The`가 불용어로 간주되지 않아서 그냥 남아있게 됨.
...
```
- `PUT ...`: 새로운 인덱스를 생성하라는 HTTP 메서드
- `mappings & properties`: 이 인덱스에 들어올 JSON 문서들의 스키마를 정의하는 블록
- `"type": "text"`: `message`라는 필드에 들어오는 데이터는 단순 문자열이 아닌 **전문 검색 대상**이라고 루씬 엔진에게 선언하는 것.
	- 이 타입이 지정되면 데이터를 통째로 디스크에 저장하지 않고 반드시 애널라이저를 거치도록 함.
	- 반대로 `"type": "keyword"` 타입을 주면 분석기를 거치지 않고 원문 그대로 1개의 토큰으로 디스크에 저장됨.

```json
{
  "mappings": {
    "properties": {
      "user_name": { "type": "text" },        // 문자열 (분석기 태워서 쪼개는 놈)
      "age": { "type": "integer" },           // 숫자 (크기 비교용)
      "created_at": { "type": "date" },       // 날짜 (시간 검색용)
      "is_active": { "type": "boolean" }      // 논리형 (True/False)
    }
  }
}
```
...
결국 핵심은, 원문이 **jumps**여도 애널라이저에 의해 **jump**라는 텀으로 색인될 수 있다는 점이다. 이 경우 `match` 쿼리는 검색어 **jumps**에도 동일한 애널라이저를 적용해 **jump**로 변환하므로 결과가 나오지만, `term` 쿼리는 검색어를 변환하지 않으므로 **jumps**로 검색하면 결과가 나오지 않고 **jump**로 검색해야 결과가 나온다.


이처럼 Elasticsearch는 `_source`에는 원문을 그대로 보관하고, 검색에 사용되는 역색인에는 애널라이저로 분석한 텀을 저장함. 
>`_source`는 검색 결과를 보여주기 위한 원문임. 역색인에서 찾아서 해당 원문을 반환하는 것.

검색어에도 같은 애널라이저가 적용되면 대소문자 변환이나 어간 추출 등을 통해 원문의 표현과 검색어가 완전히 같지 않아도 검색될 수 있음.
