...
## 접근 방식에 따른 유형
![[Pasted image 20260207010301.png]]

# 분석 방법론
- 절차, 방법, 도구와 기법, 템플릿과 산출물

# 분석 모델의 유형
- 계층적 프로세스 모델: 단계(BaseLine으로 관리) -> 태스크 -> 스탭(단기간 수행 WorkPacakge)
- 폭포수 모델: 이전 단계 완료되어야 다음 단계 진행 (Top-Down)
- 프로토타입 모델: 여러 개발 과정을 거쳐 점진적으로 완성, 위험 요소 제거에 초점
- 애자일: 짧은 개발 주기를 가지고 고객 피드백을 지속적으로 반영하여 반복적인 개발 진행

# KDD 분석 방법론
- 데이터 선택 -> 전처리 -> 변환 -> 마이닝 -> 평가
- 데이터선택: 원시데이터나 DB에서 필요한 데이터 선택
...
- **업데데**이트**모델평가전**
- 업무 이해: 업무 목적 파악, 상황 파악, 목표 설정, 프로젝트 계획 수립
...
- 전개: 전개 계획, 모니터링 및 유지보수 계획 수립, 프로젝트 종료 보고서 작성, 프로젝트 ㅣ뷰
	- 평가 -> 전개에서 위대한 실패(업무 이해로 돌아감) 발생 가능


# 빅데이터 분석 방법론
![[Pasted image 20260207013653.png]]
각 단계마다 무슨 태스크?..

# 각 단계별 태스크 내용
## 분석 기획
- Statement of Works - 구조화된 프로젝트 정의서
...
- 데이터 양, 속도, 데이터 복잡성, 분석 복잡성, 정확도/정밀도
- 정확도와 정밀도는 트레이드 오프 관계
...
베르누이분포 --N번 시행--> 이항분포
이항 분포에서 비복원 방식 사용 -> 초기화 분포
이항 분포에서 시행 횟수를 무제한 -> 포아송 분포


# 점추정 - `불효일충`
- 불편성: 추정량의 기댓값이 실제 모수와 동일 (편향 0)
- 효율성: 여러 추정량 중 분산이 작은 것이 더 효율적인 추정량
- 일치성: 표본 크기가 증가할수록 추정량이 모수에 가까워짐
- 충족성: 추정량이 모집단의 정보를 최대한 반영함
