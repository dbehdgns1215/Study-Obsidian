...
public class BOJ_1010_mult {
    // C(m, n) = mPn / n! 를 안전하게(정수로) 계산
    static long comb(int m, int n) {
        if (n < 0 || n > m) return 0;
        n = Math.min(n, m - n); // 대칭성 C(m,n)=C(m,m-n)
        long res = 1;
        for (int i = 1; i <= n; i++) {
            res = res * (m - n + i) / i; // 나눗셈이 항상 나누어떨어짐
        }
        return res;
    }
 ...
 *
 */
public class SWEA_1249_Dijkstra {

	static int N = 0, INF = Integer.MAX_VALUE;// 매 칸 깊이 한자리의 수이므로 최대9 * 칸10000 = 합은 90000 이하
	static int map[][];
