...
## 9. 일간·주간 리포트 상태별 활용 정의

본 시스템에서 일간 및 주간 리포트는 각각 **진행중(PROGRESS)** 과 **확정(CONFIRMED)** 상태에 따라 명확히 다른 목적과 책임을 가진다. 이 구분은 데이터 저장 전략뿐 아니라, AI 분석 호출 시점과 사용자 경험 설계의 기준이 된다.

---

### 9.1 일일 리포트 (Daily Report)

#### 9.1.1 일일 진행중 리포트 (DAILY · PROGRESS)

**목적**

- 당일 식사 기록에 따른 현재 상태를 실시간으로 반영
    
- 사용자에게 "오늘 현재까지의 상태"를 직관적으로 제공
    

**주요 활용**

- 오늘의 총 섭취 칼로리 및 영양소 누적치 표시
    
- 목표 대비 초과/부족 여부 확인
    
- 챗봇 질의 시 컨텍스트 데이터로 활용
...
- Redis 또는 MySQL 임시 저장
    
- 장애 시 식사 기록 기반 재계산 가능
    

---

#### 9.1.2 일일 확정 리포트 (DAILY · CONFIRMED)

**목적**

- 하루 동안의 식사 기록을 요약한 공식 기록 생성
...
### 9.2 주간 리포트 (Weekly Report)

#### 9.2.1 주간 진행중 리포트 (WEEKLY · PROGRESS)

**목적**

- 현재 주의 식사 흐름과 방향성을 중간 점검
...
- 요일별 편차 및 주중/주말 패턴 확인
    
- 챗봇 분석 및 행동 유도 UI의 입력 데이터
...
#### 9.2.2 주간 확정 리포트 (WEEKLY · CONFIRMED)

**목적**

- 일정 기간 동안의 식사 습관을 분석·평가하는 최소 의미 단위 생성
