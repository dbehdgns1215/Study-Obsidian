...
log.info("[MR Review] 시작: project={}, mr={}", projectId, mrIid);
        ...
        // ---------------------------------------------
```
- **안전지대 (Critical Section):** 락을 획득한 딱 하나의 스레드(승리자)만 이 구간에 진입하여 일을 합니다. AI 응답이 30초가 걸리든 1분이 걸리든, 이 스레드가 이 구간을 빠져나가 락을 풀기 전까지는 같은 MR 번호로 들어온 다른 이벤트들은 앞선 ④번 단계에서 튕겨 나갑니다.
