...
## 그런데...
사실상 의미 없는 작업인 것 같기도 함. 그래도 레디스 분산락을 체험해볼겸 진행해봄.
왜 의미가 없냐? -> MR 요청 자체가 여러번 보낼 수 있는 구조가 아님.

단, 굳이 의미를 찾아보자면 MR을 생성하는 순간 네트워크 지연으로 서버가 빠르게 `200 OK` 응답을 주지 못했다면 GitLab이 재시도 웹훅을 날림.

이때 댓글은 기존 댓글과 더불어 새로운 댓글까지 총 2개가 생성될 것이고, AI 호출도 2번 일어나게 됨.

동시성을 제어한다기 보단 오히려 `멱등성` 보장에 초점을 맞춘 기능이었다고 생각하면 될 것 같음.
...
#### 스레드 비유
```
- 프로그램 켜짐 -> 메인 스레드(java main 메서드 실행하는 스레드) 1명이 오픈 준비.
    
- 톰캣 스레드 풀 생성 -> 서빙 직원 200명 대기.
    
- 스프링 스레드 풀 생성 -> 요리사 8명(기본값) 주방 대기.
    
- 웹훅 들어옴 -> 서빙 직원이 주문받음 -> 주방 요리사에게 `@Async`로 던짐 -> 서빙 직원은 바로 복귀.
```